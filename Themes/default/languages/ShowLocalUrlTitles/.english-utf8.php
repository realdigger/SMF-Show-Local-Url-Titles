...
$txt['ShowLocalUrlTitles_parsebbc_sub'] = 'This setting enables the parsing of local url titles in messages, when the message is viewed. This adds database queries to each page view, making it quite inefficient, it is not recommended that you use this setting for forums where efficiency is an issue.';
$txt['ShowLocalUrlTitles_posting_sub'] = 'This setting enables the parsing of local url titles in messages, when the message is posted. This setting is recommended because the mod only has to look up each url title once (when the post is created/modified), which is far more efficient than parsing the url titles on every page view.';
$txt['ShowLocalUrlTitles_autolink_sub'] = 'This setting enables the autolinking of urls when a message is created/modified. It is necessary if you want to parse autolinked urls when a post is modified/created, rather than when it is viewed.<br />Autolink urls are urls that are not contained within url or iurl tags, instead SMF automatically detects them and makes them into links.';